...
| `modules/` | **Five** toggleable modules + option declarations in `default.nix`: `python` (py3+pyenv+pipx), `node` (Node **20**+nodenv+pnpm), `java` (JDK21), `ai` (Claude Code installer + Gemini CLI via npm), `cloud` (terraform, pulumi, aws/azure/gcloud/oci CLIs, kubectl, helm, docker) |
...
| V14 | Raw udev rules on the pogo keyboard cause an **event storm** — poll instead | 📄 (both upstreams converged on polling) | `duo watch-displays` polls sysfs at 1 Hz with 2-cycle debounce; no udev triggers on attach/detach |
